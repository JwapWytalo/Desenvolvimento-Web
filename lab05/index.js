...
const { getCampos } = require('./public/script/script');
...
app.post('/categoria-salvar', (req, res) => {
  ...
  const camposCustomizados = req.body.camposCustomizados;
  const newCategoria = {
    chave: chave,
    valor: valor,
...
app.get('/campos', (req, res) => {
    const campos = getCampos(); // Obter a lista de campos personalizados do array Campos
    res.json({ campos }); // Retornar a lista de campos personalizados em formato JSON
});
